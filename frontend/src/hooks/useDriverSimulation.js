...
// Each driver starts facing a different compass direction (evenly spread around 360°)
function buildDrivers(lat, lng) {
  return DRIVER_PROFILES.map((d, i) => ({
    ...
    status:        'available',
    isSimulated:   true,
    initialHeading: (i * Math.PI * 2) / DRIVER_PROFILES.length,
    currentLocation: {
...
// ~13 m per tick at 1.5 s interval ≈ 31 km/h city speed
const MOVE_STEP   = 0.00012;
const MAX_RADIUS  = 0.010;  // ~1.1 km from user
...
export default function useDriverSimulation({ location, originPlace, step }) {
  ...
  // Persistent heading per driver — survives position state updates
  ...
  useEffect(() => {
    ...
    drivers.forEach(d => { headingsRef.current[d._id] = d.initialHeading; });
  ...
  // Heading-based movement: each driver maintains a persistent direction, turns
  // gradually, and is pulled back toward the user when it drifts too far.
  useEffect(() => {
    ...
    nearbyDrivers.forEach(d => {
      if (d.currentLocation) {
        ...
        if (headingsRef.current[d._id] == null) {
          headingsRef.current[d._id] = d.initialHeading ?? Math.random() * Math.PI * 2;
        }
      }
    });
    ...
    intervalRef.current = setInterval(() => {
      ...
      setDriverPositions(prev => {
        ...
        for (const [id, pos] of Object.entries(prev)) {
          // Slight random turn each tick — makes movement feel organic
          headingsRef.current[id] = (headingsRef.current[id] ?? 0) + (Math.random() - 0.5) * 0.3;
          const h = headingsRef.current[id];

          let dlat = Math.cos(h) * MOVE_STEP;
          let dlng = Math.sin(h) * MOVE_STEP;

          if (center) {
            const offLat = pos.lat - center.latitude;
            const offLng = pos.lng - center.longitude;
            // Pull back if too far, and flip heading toward user
            if (Math.abs(offLat) > MAX_RADIUS) {
              dlat -= offLat * 0.2;
              headingsRef.current[id] += Math.PI * 0.4 * Math.sign(offLat);
            }
            if (Math.abs(offLng) > MAX_RADIUS) {
              dlng -= offLng * 0.2;
              headingsRef.current[id] += Math.PI * 0.4 * Math.sign(offLng);
            }
          }
  ...
  const resetDrivers = useCallback((lat, lng) => {
    ...
    fresh.forEach((d, i) => {
      ...
      headingsRef.current[d._id] = (i * Math.PI * 2) / fresh.length;
    });
    driversSeeded.current = true;
